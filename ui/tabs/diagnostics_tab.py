...
from PyQt6.QtWidgets import (
    ...
    QLabel, QGroupBox, QTableView, QScrollArea, QFrame
)
...
class DiagnosticsTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        por_layout = QHBoxLayout(por_group)
        
        self.phie_hist = HistogramPlot()
        self.phie_hist.setMinimumHeight(250)
        por_layout.addWidget(self.phie_hist, stretch=2)
        ...
        por_layout.addWidget(self.phie_stats_table, stretch=1)
    ...
    def update_display(self):
        ...
        # Would need to calculate statistical values here
        self.shale_stat_label.setText("(Run calculation to compare)")
        ...
        if calculated_shale and isinstance(calculated_shale, dict):
            ...
            threshold = calculated_shale.get('shale_threshold_used', calculated_shale.get('threshold', 0))
            before = calculated_shale.get('shale_points_before', calculated_shale.get('shale_points', '?'))
        ...
        # POROSITY VALIDATION
        # =====================================================================
        phie_cols = ['PHIE_D', 'PHIE_N', 'PHIE_S', 'PHIE_DN', 'PHIE']
        available_phie = [c for c in phie_cols if c in results.columns and results[c].notna().sum() > 0]
        ...
        if available_phie:
            ...
            # Histogram (show PHIE if available)
            if 'PHIE' in available_phie:
                self.phie_hist.plot_histogram(results['PHIE'].dropna(), "PHIE Distribution")
            
            # Warnings
            warnings = []
            phie = results.get('PHIE', pd.Series())
            if len(phie) > 0:
                high_phie = (phie > 0.45).sum()
                low_phie = (phie < 0).sum()
                if high_phie > 0:
                    warnings.append(f"⚠️ {high_phie} points with PHIE > 0.45")
                if low_phie > 0:
                    warnings.append(f"⚠️ {low_phie} points with PHIE < 0 (negative)")
            
            if warnings:
                self.phie_warnings.setText("\n".join(warnings))
                self.phie_warnings.setStyleSheet("color: orange;")
            else:
                self.phie_warnings.setText("✅ No PHIE outliers detected")
        ...
        available_sw = [c for c in sw_cols if c in results.columns and results[c].notna().sum() > 0]
        ...
        if available_sw:
            ...
            # Histogram
            if 'SW_INDO' in available_sw:
                self.sw_hist.plot_histogram(results['SW_INDO'].dropna(), "Sw (Indonesian)")
        
        # =====================================================================
        # PERMEABILITY VALIDATION
        # =====================================================================
        perm_cols = ['PERM_TIMUR', 'PERM_WR']
        available_perm = [c for c in perm_cols if c in results.columns and results[c].notna().sum() > 0]
        ...
        if available_perm:
            ...
            if 'PHIE' in results.columns and 'PERM_TIMUR' in results.columns:
                ...
                self.perm_crossplot.plot_crossplot(
                    results['PHIE'], perm_log,
                    color_data=results.get('VSH'),
                    x_label='PHIE (v/v)', y_label='log10(k) [mD]',
        ...
        if summary:
            ...
            warnings = []
            if net_pay < 1:
                warnings.append(f"⚠️ Net Pay ({net_pay:.1f} ft) < 1 ft - may be too thin")
        ...
        if self.model.core_data is not None:
            ...
            self.core_depth_card.set_value(f"{depth_range[0]:.0f} - {depth_range[1]:.0f} ft")
            ...
            if 'DEPTH' in results.columns and 'PHIE' in results.columns:
                ...
                if por_result:
                    ...
                    stats_data = [
                        ...
                        {'Metric': 'R²', 'Value': f"{por_result.r_squared:.3f}" if por_result.r_squared else "N/A"},
                        {'Metric': 'Spearman ρ', 'Value': f"{por_result.spearman_rho:.3f}"},
                    ]
                    self.core_por_stats_model.set_dataframe(pd.DataFrame(stats_data))
                    
                    # Crossplot
                    matched = core.get_matched_data(log_depth, log_phie=log_phie)
                    if 'CORE_POROSITY' in matched.columns and 'LOG_PHIE' in matched.columns:
                        valid = matched.dropna(subset=['CORE_POROSITY', 'LOG_PHIE'])
                        if len(valid) > 0:
                            self.core_por_crossplot.plot_crossplot(
                                valid['CORE_POROSITY'], valid['LOG_PHIE'],
                                x_label='Core Porosity', y_label='Log PHIE',
                            ...
                            if self.core_por_crossplot.figure.axes:
                                ...
                                lims = [0, max(valid['CORE_POROSITY'].max(), valid['LOG_PHIE'].max()) * 1.1]
                ...
                if 'PERM_TIMUR' in results.columns:
                    ...
                    if perm_result:
                        ...
                        stats_data = [
                            ...
                            {'Metric': 'Bias (linear)', 'Value': f"{perm_result.bias_linear:.2f}"},
                            {'Metric': 'MAE (linear)', 'Value': f"{perm_result.mae_linear:.2f}"},
                            {'Metric': 'RMSE (linear)', 'Value': f"{perm_result.rmse_linear:.2f}"},
                            {'Metric': 'MAE (log10)', 'Value': f"{perm_result.mae_log10:.3f}"},
                            {'Metric': 'Spearman ρ', 'Value': f"{perm_result.spearman_rho:.3f}"},
                        ]
                        self.core_perm_stats_model.set_dataframe(pd.DataFrame(stats_data))
                        
                        # Crossplot
                        matched = core.get_matched_data(log_depth, log_perm=log_perm)
                        if 'CORE_PERM' in matched.columns and 'LOG_PERM' in matched.columns:
                ...
                if por_result and por_result.r_squared and por_result.r_squared < 0.5:
                    warnings.append(f"⚠️ Porosity R² = {por_result.r_squared:.2f} (low correlation)")
                ...
                if warnings:
                    ...
                    self.core_warnings.setText("✅ Core validation within acceptable range")
    ...
    def _plot_depth_track_with_core(self, core, results):
        ...
        if 'PHIE' in results.columns:
            log_phie = results['PHIE'].values
            ax1.plot(log_phie, log_depth, color='#00CED1', linewidth=1, label='Log PHIE')
        
        # Overlay core porosity
        try:
            core_depths, core_por = core.get_core_porosity()
            if len(core_depths) > 0:
                ax1.scatter(core_por, core_depths, c='#006666', marker='D', 
                           s=40, zorder=5, label='Core Porosity', edgecolors='white', linewidths=0.5)
        except:
            pass
        ...
        if 'PERM_TIMUR' in results.columns:
            ...
            ax2.plot(log_perm, log_depth, color='#FF6347', linewidth=1, label='Log Perm')
        
        # Overlay core permeability
        try:
            core_depths, core_perm = core.get_core_permeability()
            if len(core_depths) > 0:
                ax2.scatter(core_perm, core_depths, c='#CC0000', marker='D', 
                           s=40, zorder=5, label='Core Perm', edgecolors='white', linewidths=0.5)
        except:
            pass
